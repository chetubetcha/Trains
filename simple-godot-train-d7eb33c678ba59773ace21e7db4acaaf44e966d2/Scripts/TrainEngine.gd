...
func _ready():
	"""Global.register_engine(self)""" #Use this to send Engine to UI
	engine_instance = self
	is_lead_vehicle = true
	set_and_propagate_velocity(velocity,velocity_multiplier)
	hide_and_disable_buttons_for_engine()
	_update_frictions()
	connect("towed_mass_changed", self, "_on_towed_mass_changed")
	addToTrain(fullTrain)

func _on_towed_mass_changed(mass_delta, emitter):
	_update_frictions()

# Emit a signal to update the HUD
func _process(delta):
	#print("Full Train: ",fullTrain)
	#print(is_lead_vehicle)
	if SELECTED:
...
# This should be in your engine's script
func update_total_mass():
	var vehicles = get_all_vehicles_in_train()
	total_mass = 0  # Reset
	for vehicle in vehicles:
		total_mass += vehicle.mass  # Assuming every vehicle has a 'mass' property

	# Now total_mass contains the sum of the masses of all vehicles in the train
...
func _move_with_friction(delta):
	...
	if applied_force == 0 and abs(velocity) < resistance / total_mass * delta:
		velocity = 0
	else:
		if velocity > 0:
	...
	# Check if the velocity has changed and propagate it to all connected vehicles
...
onready var attach_front = $Body/AttachFront
onready var attach_back = $Body/AttachBack
onready var detach_front = $Body/DetachFront
onready var detach_back = $Body/DetachBack

func hide_and_disable_buttons_for_engine():
	attach_front.hide()
	attach_back.hide()
	detach_front.hide()
	detach_back.hide()
